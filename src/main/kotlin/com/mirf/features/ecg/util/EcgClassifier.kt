...
object EcgClassifier {
    ...
    val mirfToTfNotations = hashMapOf(
    ...
    fun classify(series: BufferedImageSeries): EcgDiagnosis {
        ...
        for (img in series.images) {
            ...
            val res = tfModel.runModel(imgArr, 1, 128, 128, 1)

            classes[res.indices.maxByOrNull { res[it] } ?: -1] += 1
        }

        val arrhythmiaToNumber = HashMap<EcgArrhythmiaType, Int>()

        for (type in mirfToTfNotations) {
            arrhythmiaToNumber[type.key] = classes[type.value]
            println("${type.key} ${classes[type.value]}")
        }
